...
public class AppScanningSteps {
    ...
    private final static String ZAP_CONTEXT_NAME= "Contexte par défaut";
    ...
    public void createNewScanSession() {
        ...
        navigateAppIfNotAlreadyNavigated();
    }
    ...
    @Given("prepare to scan (\\S+) weakness")
	public void enablePolicyy(String policyName) throws Exception {
		deleteAlerts();
		disableAllScanners();
	...
	public void outputValide() throws IOException {
    	//Url of the application
    	String url = "url";
    	//Insertion of an alphabetic value in the quantity field
    	app.getdriver().get(url+"?quantity_16=a&update=Update+Basket");
    	//Assert that the appliation does not return debug information
//		
    	assertThat(! (app.getdriver().getPageSource().contains("500")||
//				(app.getdriver().getPageSource().contains("exception"))) , is(true));
//		
    	assertThat(app.getdriver().getPageSource(), anyOf( CoreMatchers.containsString("error"),CoreMatchers.containsString("forbidden"),CoreMatchers.containsString("unauthorized")));
    	//HTTP status(503, 408 for quiescent state, the others for Unauthorized access)
    	URL obj = new URL(url);
		HttpURLConnection con = (HttpURLConnection) obj.openConnection();
		con.setRequestMethod("GET");
		assertThat(con.getResponseCode(), anyOf(is(200),is(503),is(408),is(401),is(403),is(405),is(409)));
    ...
    @And("^the application is injected$")
    public void InjectApp() {
        
            if (!(app instanceof INavigable))
                throw new RuntimeException("The application must implement the 'INavigable' interface to be navigable");
            app.enableHttpLoggingClient();
            log.debug("Navigating");
            ((INavigable) app).navigateInj();
            
        
    }
    @Then ("the application validates input")
    public void inputValidation() throws Throwable{
