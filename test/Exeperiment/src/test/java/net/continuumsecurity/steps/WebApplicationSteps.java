...
import net.continuumsecurity.examples.ropeytasks.BodgeitApplication;
...
public class WebApplicationSteps {
	...
	BodgeitApplication app;
	LoggingProxy proxy;
	
	@Rule
	ErrorCollector coll = new ErrorCollector();

	public WebApplicationSteps() {
		app = new BodgeitApplication();
	}
	...
	public void openLoginPage() {
		...
		app.getdriver().get(Config.getInstance().getBaseUrl()+"login.jsp");
	}
	...
	public void forcelogin() {
		// recuperer le driver selenium driver driver.get(url);
		// objectif: ecrire test selenium pour injecter mauvaise entrée dans la page login
		// remplir le champs username avec n'importe quel username 
		// remplir le champs password n'importe quel password suivi de ('or'1'='1) (voir sujet)
		// click submit
    	app.getdriver().findElement(By.id("username")).sendKeys("test");
    	app.getdriver().findElement(By.id("password")).sendKeys("test'or'1'='1");
    	app.getdriver().findElement(By.id("submit")).click();
	...
	public void statusCode2xx() {
		// recuperer le driver selenium avec app.getdriver()
		// objectif: ecrire test selenium pour injecter mauvaise entrée comme  "'or'1'='1" sur le bon lien (voir sujet)
		// verifier que l'injection sql ne se fait pas 
		// (pas de succes de login par exemple)
		assertThat(! app.getdriver().getPageSource().contains("logged in successfully") , is(true));
		
	}
    @Given("vulnerable page with executed script$")
	public void xssedpage() {
		
		// recuperer le driver selenium avec app.getdriver()
		// objectif: ecrire test selenium pour injecter un script
		//naviguez vers 
    	//http://192.168.109.218:8080/bodgeit/search.jsp?q=<script>var link=document.getElementsByTagName('a');link[0].href='http://google.com'</script>
		//afin de redériger le lien (Home) vers un site (http://google.com/) (voir sujet)
	...
	public void navigatetoInsertedLink() throws IOException {
		
		// recuperer le driver selenium avec app.getdriver()
		// ecrire un test selenium qui pointe vers le lien Home de l'application
	...
	public void changedLink() {
    	// recuperer le driver selenium avec app.getdriver()
    	// recuperer le titre de la page avec le driver .getTitle()
    	//Faire une assertion pour vérifier que le titre de la page est bien "The BodgeIt Store"
	...
	@When("the user bad logs in")
	public void loginWithSetCredentialsbad() {
		assert World.getInstance().getCredentials() != null;
		openLoginPage();

		((ILogin) app).login(World.getInstance().getUserPassCredentials());
	}
	...
	@When("^the user logsy in from a fresh login page$")
	public void loginFromFreshPage() throws IOException {
		createApp();
		openLoginPage();
		loginWithSetCredentials();
	...
	public void loginSucceeds() {
		assertThat("The user is logged in", ((ILogin) app).isLoggedIn(), is(false));
	}
	...
	@Given("^the user logsx in from a fresh login page\\ (\\d+) times$")
	...
	@Given("the HTTP request-response containing the bad credentials is selected")
	public void findRequestWithBadPassword() {
		UserPassCredentials credentials = World.getInstance().getUserPassCredentials();
		List<HarEntry> all = getProxy().getHistory();
		List<HarEntry> requests = getProxy().findInRequestHistory("%27OR%271%3D1%27");
	...
	@When("^the passwordd field is inspected$")
