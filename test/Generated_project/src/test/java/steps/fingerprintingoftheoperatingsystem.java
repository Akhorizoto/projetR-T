package steps;
import cucumber.api.java.en.Given;
import cucumber.api.java.en.Then;
import cucumber.api.java.en.When;


// In order to create a valid file injection, the attacker needs to know what the underlying OS is.

public class fingerprintingoftheoperatingsystem {
@Given("prepare to Fingerprinting of the operating system")
public void prefingerprintingoftheoperatingsystem(){
// 
// Program must allow for user controlled variables to be applied directly to the filesystem
}
@When("Try to Fingerprinting of the operating system")
public void tryfingerprintingoftheoperatingsystem(){
//  Try one of the following techniques :
   //1.  Port mapping. Identify ports that the system is listening on, and attempt to identify inputs and protocol types on those ports.
   //2.  TCP/IP Fingerprinting. The attacker uses various software to make connections or partial connections and observe idiosyncratic responses from the operating system. Using those responses, he attempts to guess the actual operating system.
   //3.  Induce errors to find informative error messages
}@Then("Assert the success of Fingerprinting of the operating system")
public void assfingerprintingoftheoperatingsystem(){
//  Assert one of the following indications :
   //  -Operating environment (operating system, language, and/or middleware) is correctly identified.
    //  -The target software accepts connections via the network.
}
}